...
public class ATM {
    ...
    public boolean withdrawCash(double amount) {
        if (card.getBalance("PLN") >= amount && amount % 10 == 0 && amount <= 4000 && amount > 0) {
    ...
    public boolean depositCash(double amount) {
        if (amount % 10 == 0 && amount <= 20000 && amount > 0) {
            card.setBalance("PLN", card.getBalance("PLN") + amount);
    ...
    public boolean exchangeCurrency(double amount, String sourceCurrency, String targetCurrency) throws Exception {
        double exchangeRate;
